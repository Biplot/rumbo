...
async function registrar(page, email) {
  ...
  await page.evaluate(() => { STATE.settings.onboarded = true; STATE.settings.introVersion = 10; STATE.settings.tutorial = { vistos: {}, mision: "oculta", auto: false, ts: 0 }; ONB_ACTIVE = false; closeModal(); saveState(); });
